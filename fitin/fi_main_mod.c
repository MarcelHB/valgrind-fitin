...
static void initTData(void) {
    ...
    tData.load_states = VG_(newXA)(VG_(malloc),
                                   ...
                                   VG_(free),
                                   sizeof(LoadState));
    tData.monitorables = AVLTree_new(cmpMonitorable, sizeof(Monitorable));
...
void fi_stop_using_mem_stack(const Addr a, const SizeT len) {
    AVLTree_It *it = AVLTree_iterator(tData.monitorables);
    void *data = NULL;
    
    while((data = AVLTree_next(it)) != NULL) {
        Monitorable *mon = (Monitorable*)data;
...
static Word VEX_REGPARM(3) preLoadHelper(toolData *td, 
    ...
    Monitorable key;
    Word state_list_size = VG_(sizeXA)(td->load_states);
    ...
    if(td->ignore_monitorables) {
        ...
        Monitorable *mon = AVLTree_find(td->monitorables, &key);
...
static IRSB *fi_instrument(VgCallbackClosure *closure,
    ...
    IRDirty *di;
    int i = 0;
    AVLTree *loads = NULL, *replacements = NULL;
    Bool monitor_sb = False, monitor_check = False;

    if(!tData.runtime_active) {
        return sbIn;
    }

    loads = AVLTree_new(fi_reg_compare_loads, sizeof(LoadData));
    replacements = AVLTree_new(fi_reg_compare_replacements, sizeof(ReplaceData));
    ...
    AVLTree_destroy(replacements);
    AVLTree_destroy(loads);
...
static void fi_fini(Int exitcode) {
    ...
    AVLTree_destroy(tData.monitorables);
...
static Bool fi_handle_client_request(ThreadId tid, UWord *args, UWord *ret) {
    ...
    switch(args[0]) {
        ...
        case VG_USERREQ__MON_MEM: {
            ...
            Monitorable *other_mon = (Monitorable*)AVLTree_find(tData.monitorables, &mon);
            ...
            if(other_mon != NULL) {
                ...
                AVLTree_add(tData.monitorables, &mon);
            }
...
static void fi_reg_on_mem_read(CorePart part, ThreadId tid, const HChar *s,
    ...
    if(tData.runtime_active && part == Vg_CoreSysCall) {
        ...
        for(; mem_offset <= size; ++mem_offset) {
            ...
            Monitorable *mon = AVLTree_find(tData.monitorables, &key);
