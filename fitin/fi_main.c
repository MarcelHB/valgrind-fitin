...
static Bool fi_handle_client_request(ThreadId tid, UWord *args, UWord *ret) {
    ...
    switch(args[0]) {
        ...
        case VG_USERREQ__BP: {
            if(tData.available_callbacks & CALLBACK_BP) {
                lua_getglobal(tData.lua, "breakpoint");
                lua_pushinteger(tData.lua, args[1]);
                lua_pushinteger(tData.lua, args[2]);
                lua_pushinteger(tData.lua, args[3]);
                lua_pushinteger(tData.lua, args[4]);
                lua_pushinteger(tData.lua, args[5]);

                if(!lua_pcall(tData.lua, 5, 0, 0) == 0) {
                    VG_(printf)("LUA: %s\n", lua_tostring(tData.lua, -1));
                }
            }
            break;
