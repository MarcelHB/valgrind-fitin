...
#define STAT_FUNC (int (*)(const char *, struct vg_stat *)) VG_(stat)
#define STAT_STRUCT struct vg_stat
#else
...
#ifdef FITIN_WITH_LUA
#define S_ISDIR(m) VKI_S_ISDIR(m)
#define S_ISLINK(m) VKI_S_ISLINK(m)
#define S_ISSOCK(m) VKI_S_ISSOCK(m)
#define S_ISFIFO(m) VKI_S_ISFIFO(m)
#define S_ISCHR(m) VKI_S_ISCHR(m)
#define S_ISBLK(m) VKI_S_ISBLK(m)
#define S_ISREG(m) VKI_S_ISREG(m)
#define S_ISLNK(m) VKI_S_ISLNK(m)
#define S_IRUSR VKI_S_IRUSR
#define S_IWUSR VKI_S_IWUSR
#define S_IXUSR VKI_S_IXUSR
#define S_IRGRP VKI_S_IRGRP
#define S_IWGRP VKI_S_IWGRP
#define S_IXGRP VKI_S_IXGRP
#define S_IROTH VKI_S_IROTH
#define S_IWOTH VKI_S_IWOTH
#define S_IXOTH VKI_S_IXOTH
#define F_SETLK VKI_F_SETLK
...
/* inode protection mode */
static void push_st_mode (lua_State *L, STAT_STRUCT *info) {
        lua_pushstring (L, mode2string (info->mode));
}
/* device inode resides on */
static void push_st_dev (lua_State *L, STAT_STRUCT *info) {
        lua_pushnumber (L, (lua_Number)info->dev);
}
/* inode's number */
static void push_st_ino (lua_State *L, STAT_STRUCT *info) {
        lua_pushnumber (L, (lua_Number)info->ino);
}
/* number of hard links to the file */
static void push_st_nlink (lua_State *L, STAT_STRUCT *info) {
        lua_pushnumber (L, (lua_Number)info->nlink);
}
/* user-id of owner */
static void push_st_uid (lua_State *L, STAT_STRUCT *info) {
        lua_pushnumber (L, (lua_Number)info->uid);
}
/* group-id of owner */
static void push_st_gid (lua_State *L, STAT_STRUCT *info) {
        lua_pushnumber (L, (lua_Number)info->gid);
}
/* device type, for special file inode */
static void push_st_rdev (lua_State *L, STAT_STRUCT *info) {
        lua_pushnumber (L, (lua_Number)info->rdev);
}
/* time of last access */
static void push_st_atime (lua_State *L, STAT_STRUCT *info) {
        lua_pushnumber (L, info->atime);
}
/* time of last data modification */
static void push_st_mtime (lua_State *L, STAT_STRUCT *info) {
...
/* blocks allocated for file */
static void push_st_blocks (lua_State *L, STAT_STRUCT *info) {
        lua_pushnumber (L, (lua_Number)info->blocks);
}
/* optimal file system I/O blocksize */
static void push_st_blksize (lua_State *L, STAT_STRUCT *info) {
        lua_pushnumber (L, (lua_Number)info->blksize);
}
#endif
...
static void push_st_perm (lua_State *L, STAT_STRUCT *info) {
    lua_pushstring (L, perm2string (info->mode));
}
#else
static void push_st_perm (lua_State *L, STAT_STRUCT *info) {
